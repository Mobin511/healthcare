    ...
    chrome.alarms.onAlaram.addListener((alarm) => {
        if (alarm.name === "health_care") {
            chrome.notifications.creat({
                    type: "basic",
                    iconUrl: "alarm.png",
                    title: "health care!",
    ...
    chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
        try {
            if (request.action === "reset") {
                chrome.alarms.clear("health_care", () => {
                    if (chrome.runtime.lastError) {
            ...
            console.error("Error is message handler:", error);
            sendResponse({
                success: false,
                error: error.message
            });
    ...
    function createAlarm(minutes) {
        ...
        chrome.alarms.clear("health_care", () => {
            chrome.alarms.create("health_care", {
            ...
            chrome.alarms.get("health_care", (alarm) => {
